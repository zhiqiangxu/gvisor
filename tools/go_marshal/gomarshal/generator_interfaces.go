...
func (g *interfaceGenerator) emitMarshallable() {
	...
	g.emit("func (%s *%s) MarshalBytes(order encbin.ByteOrder, dst []byte) {\n", g.r, g.typeName())
	g.inIndent(func() {
		g.forEachField(fieldDispatcher{
			primitive: func(n, t *ast.Ident) {
				if n.Name == "_" {
	...
	g.emit("func (%s *%s) UnmarshalBytes(order encbin.ByteOrder, src []byte) {\n", g.r, g.typeName())
	g.inIndent(func() {
		g.forEachField(fieldDispatcher{
			primitive: func(n, t *ast.Ident) {
				if n.Name == "_" {
	...
	g.emit("func (%s *%s) MarshalUnsafe(order encbin.ByteOrder, dst []byte) {\n", g.r, g.typeName())
	g.inIndent(func() {
		if thisPacked {
	...
	g.emit("func (%s *%s) UnmarshalUnsafe(order encbin.ByteOrder, src []byte) {\n", g.r, g.typeName())
	g.inIndent(func() {
		if thisPacked {
			g.recordUsedImport("safecopy")
			g.recordUsedImport("unsafe")
